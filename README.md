...
The initial room area is template metadata; independently editing room walls does not maintain BIM-style room constraints. Section cuts clip the display without producing capped construction sections. SVG export is a footprint projection of visible boxes, not an associative drawing-sheet system. OBJ/STL export uses millimetres and Z-up; GLB uses metres and Y-up. Exported mesh formats do not preserve the editable project history; use JSON for editing roundtrips. Mirroring preserves symmetric box/cylinder shape, while asymmetric custom profiles require manual checking. Gizmo scaling that would shear rotated parts or invalidate a jointed assembly is rejected; resize furniture through its configuration or dimension individual boards before attaching joints.

Material rates are editable planning assumptions. Estimates exclude labour, taxes, hardware, supplier pricing, optimized cutting/nesting and manufacturing validation. The furniture clearance dialog checks current and closed geometry and joint opening paths, including hidden parts. Motion uses oriented bounds and can stop conservatively for shaped objects; the static audit checks actual intersection volume for closed shaped solids. Mounting reference fittings are excluded, while handles, legs and feet participate. See [clearance and manual assembly](documentation/CLEARANCE-AND-MANUAL-BUILD.md) for coverage and limits. Before fabrication, verify supplier hardware, required tolerances, loads, edges and material specifications.
