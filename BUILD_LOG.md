...
77. Reproduced the rectangle preview regression: drawPreview expanded the two diagonal control points into an outline, then drawingGeometry interpreted its first two adjacent corners as the diagonal, collapsing the preview into a line. Removed the duplicate expansion so preview and committed shape use the same original controls. The new test failed before the fix and passed afterward across XY/XZ/YZ and four drag directions, checking four unique corners, dimensions, no premature face/history entry, final face creation and 18 mm extrusion. All 13 targeted drawing/manual-workflow tests and the production build passed. Browser inspection confirmed four visible preview edges with zero project objects before the second click, then exactly one rectangle face afterward.

## 78. Material, sweep, snapping and solid corrections; bounded furniture AI (10 September, before 15:40 IST)

- Material selection now navigates to the visible palette. Added independent custom colours and hex entry. Imported material colour tints are applied, and painting no longer resets the selected swatch when changing objects. Browser verification changed a rectangle to #245ee8 and reassigned its layer.
- Follow Me uses a rectangle's broad face on XY/XZ/YZ and can use a selected solid face. Validation failures keep the dialog open. Boolean replacements check commit success and retain nested group ancestry. New dimensioned solids start beside existing furniture.
- Elevated first-point snaps establish the drawing plane height. Shaped/profile/imported mesh snapping uses feature edges rather than cylinder bounding-box corners. Ordinary gizmo movement remains grid-based; point-to-point movement handles exact feature alignment.
- Added a five-slot server-only Gemini furniture planner with explicit Free-tier confirmation, model metadata verification, bounded prompt/output sizes, persistent shared attempt/token reservations, credential/service fallback and quota-stop behaviour. Real text generation remains pending key/billing setup. One existing local key passed metadata verification; four requested slots remain empty; no cloud keys were published.
- Full ordinary suite: 112/112 passed. Production build passed. Public Cloudflare integration suite: 6/6 passed. Current version 9cd7ad1f-52d0-4aea-b672-c9d09d2a28dc; asset index-DC9IQ_TS.js.
- Detailed jury walkthrough: documentation/JURY-FURNITURE-GUIDE.md. It records exact manual cabinet dimensions, supported tool workflows, AI setup, verification evidence and explicit remaining limitations. No claim of exhaustive SketchUp parity or perfect arbitrary-model behaviour.
...
## AI Design Studio cloud connection fix
...
Replaced the legacy hardcoded unconfigured status/generation routes with a shared structured Gemini transport. Both cloud dialogs route to the same existing Durable Object budget. Provider usage history is recorded even when response validation fails; no project content is sent to Gemini beyond whitelisted planning fields. Added regression coverage for exact room geometry, what-if intent, non-destructive wardrobe preview, incomplete-output accounting, shared cloud routing and budget refusal. Updated the jury guide and Downloads copy.
