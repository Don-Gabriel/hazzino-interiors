...
This is a functional furniture modeller with original Hazzino styling. It is not verified inch-by-inch parity with SketchUp 2026. Connected edge/face topology, native shared component instances, extension compatibility, DWG/DXF/IFC and native SKP round trips remain unimplemented. General inference and Push/Pull interactions cover a narrower scope than native SketchUp. Outer Shell currently behaves as union. Boolean edits do not preserve all original texture UVs or per-face materials. Gizmo transforms that would shear rotated parts or scale a jointed assembly are rejected. Regeneration after manual part edits rebuilds from the saved configuration.

Door/drawer movement now uses rigid hinge/slide joints and swept obstruction checks. Enclosed drawers require their obstructing doors to clear first; drawers block door closing until retracted. Manually drawn solids can use the same joints. Sliding layouts whose drawers cannot be reached from either side are rejected by the builder. Use **Furniture → Hinges, slides & clearance** for individual control and a project audit. See [the detailed guide](CLEARANCE-AND-MANUAL-BUILD.md).

Presets use butt-jointed carcasses, applied backs and a 13 mm runner allowance; enclosed fronts receive additional setback, side clearance and spacer boards when needed. These are geometric constraints rather than manufacturer-specific hinge dynamics or structural certification. The app does not calculate loads, deflection or gravity, or generate CNC postprocessor output.
